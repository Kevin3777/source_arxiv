...
def build_hf_dataset(
    ...
    # 为论文摘要数据集创建自定义处理逻辑
    def process_paper_abstract(sample):
        # 使用标题作为"问题"，摘要作为"答案"
        question = f"Summarize the paper: {sample['title']}"
        answer = sample['text']
        
        # 添加一个虚拟的 URL（可以是论文链接或其他）
        url = f"https://arxiv.org/abs/{sample.get('enc-paper-str', 'unknown')}"
    ...
    # 应用自定义处理
    processed_dataset = hf_dataset.map(process_paper_abstract)
    ...
    if ('train' in split) or 'ppl_val' in split:
        data_cls = PackingURLDataset
    elif 'url_val' in split or 'fact' in split or 'qa' in split:
        data_cls = NoPackingURLDataset
    else: 
        raise ValueError(f'Unknown split: {split}')
            
    dataset = data_cls(hf_dataset=processed_dataset,
                       tokenizer=tokenizer,
...
# 添加函数，因为输入是 list 这种非 tensor l
def custom_collate(batch):
    # 将 batch 中每条样本以 key 为维度组合成 list，适用于非 tensor 类型的数据结构
