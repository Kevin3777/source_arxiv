...
def download_and_save_data(cfg):
    ...
    print(f"Using local dataset files from {cfg.data.text_data_path}")
    
    # Path to your local jsonl files
    pretrain_file = os.path.join(cfg.data.text_data_path, 'train', 'train.jsonl')
    qa_train_file = os.path.join(cfg.data.text_data_path, 'qa', 'qa_train.jsonl')
    qa_val_file = os.path.join(cfg.data.text_data_path, 'qa', 'qa_val.jsonl')
    
    # Check if files exist
    if not os.path.exists(pretrain_file):
        raise FileNotFoundError(f"Pretrain file not found: {pretrain_file}")
    if not os.path.exists(qa_train_file):
        raise FileNotFoundError(f"QA train file not found: {qa_train_file}")
    if not os.path.exists(qa_val_file):
        raise FileNotFoundError(f"QA val file not found: {qa_val_file}")
    ...
    # Create datasets
    pretrain_dataset = load_jsonl(pretrain_file)
    qa_train_dataset = load_jsonl(qa_train_file)
    qa_val_dataset = load_jsonl(qa_val_file)
    
    # Create dataset dicts
    pretrain_data = DatasetDict({"train": pretrain_dataset})
    qa_data = DatasetDict({"qa_train": qa_train_dataset, "qa_val": qa_val_dataset})
