...
def preprocess_data(cfg):   
    ...
    qa_data_path = cfg.data.get('qa_data_path', os.path.join(os.path.dirname(text_data_path), 'qa'))
    ...
    return_code = subprocess.run([
        ...
        "--in_domain_qa_data_path", "/root/autodl-tmp/intrinsic-source-citation/dataset/ours/qa/qa_train",
...
def prepare_train_config(cfg, paths_info):
    ...
    train_cfg.seed = cfg.get('global_seed', 17) 
    
    
    train_cfg.global_train_batch_size = getattr(cfg.train, 'device_train_microbatch_size', 64) * getattr(cfg.train, 'device_eval_batch_size', 64)
    
    # 如果没有指定具体的批次大小，使用默认值
    if not hasattr(train_cfg, 'device_train_microbatch_size'):
        train_cfg.device_train_microbatch_size = 4
    
     # 确保 optimizer 配置存在
    if 'optimizer' not in train_cfg:
        train_cfg.optimizer = {}
    ...
    # 如果配置中没有指定，使用默认值
    if not hasattr(train_cfg.optimizer, 'lr'):
        train_cfg.optimizer.lr = 8.0e-5  # 从原配置文件中获取默认值
    
    if not hasattr(train_cfg.optimizer, 'weight_decay'):
        train_cfg.optimizer.weight_decay = 0.02  # 从原配置文件中获取默认值
    ...
    for dl in train_cfg.dataloaders[1:]:
        ...
        dl.dataset.path = "/root/autodl-tmp/intrinsic-source-citation/dataset/ours/qa"
